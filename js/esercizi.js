...
// Upload e ridimensionamento immagine
...
async function salvaEsercizio() {
  ...
  try {
    if (idEsercizioCorrente) {
      await update(ref(db, `portieri_esercizi/${idEsercizioCorrente}`), dati);
...
async function eliminaEsercizio() {
  ...
  try {
    await remove(ref(db, `portieri_esercizi/${idEsercizioCorrente}`));
