...
// ============================================
// CARICAMENTO LIBRERIA ESERCIZI
// ============================================
async function caricaLibreriaEsercizi() {
  try {
    const snap = await get(ref(db, "portieri_esercizi"));
...
function renderLista() {
      ...
      return `
        <div class="allenamento-card" data-id="${id}" style="${futuro ? "border-left:4px solid var(--verde);" : "opacity:0.85;"}">
          ...
          ${esercizi.map(e => {
            ...
            return `
              <div class="esercizio-riga apri-desc-esercizio" data-nome="${e.nome.replace(/"/g, "&quot;")}" data-desc="${desc.replace(/"/g, "&quot;")}">
                <span>${e.nome}</span>
  ...
  // Tocco/click su un esercizio: mostra la descrizione (funziona anche su telefono)
  listaAllenamenti.querySelectorAll(".apri-desc-esercizio").forEach(riga => {
    riga.addEventListener("click", () => {
      document.getElementById("titoloDescEsercizio").textContent = riga.dataset.nome;
      document.getElementById("testoDescEsercizio").textContent = riga.dataset.desc;
      document.getElementById("overlayDescEsercizio").classList.add("attivo");
    });
  });
}

document.getElementById("btnChiudiDescEsercizio").addEventListener("click", () => {
  document.getElementById("overlayDescEsercizio").classList.remove("attivo");
});
document.getElementById("overlayDescEsercizio").addEventListener("click", (e) => {
  if (e.target.id === "overlayDescEsercizio") e.target.classList.remove("attivo");
});
...
async function salvaAllenamento() {
  ...
  try {
    if (idAllenamentoCorrente) {
      await update(ref(db, `portieri_trainings/${idAllenamentoCorrente}`), dati);
      allenamentiCache[idAllenamentoCorrente] = dati;
    } else {
      const nuovoRef = push(ref(db, "portieri_trainings"));
...
async function eliminaAllenamento() {
  ...
  try {
    await remove(ref(db, `portieri_trainings/${idAllenamentoCorrente}`));
...
Genera una sessione di allenamento completa specifica per portieri, in base alle indicazioni del coach.
Regole:
- Crea tra 4 e 6 esercizi in totale, coprendo aree diverse (riscaldamento, tecnica specifica, gioco).
  ...
  if (campoSalvaEserciziAI.checked) {
    ...
    for (const es of allenamentoGeneratoAI.esercizi) {
      ...
      if (nomeNorm && !nomiEsistenti.has(nomeNorm)) {
        try {
          const nuovoRef = push(ref(db, "portieri_esercizi"));
